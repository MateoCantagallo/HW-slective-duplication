...
//ADDED MAT FOR DYNAMIC INSTRUCTION COUNTING
#define STATIC_INST_HASH_SIZE 65536
#define MAX_MNEMONIC_LEN 16

typedef struct StaticInstEntry {
    md_addr_t addr;
    unsigned long long count;
    char mnemonic[MAX_MNEMONIC_LEN];
    struct StaticInstEntry* next;
} StaticInstEntry;

static StaticInstEntry* static_inst_table[STATIC_INST_HASH_SIZE] = {0};
static unsigned long long total_dynamic_fetches = 0;

static void free_static_inst_table() {
    for (int i = 0; i < STATIC_INST_HASH_SIZE; i++) {
        StaticInstEntry* e = static_inst_table[i];
        while (e) {
            StaticInstEntry* next = e->next;
            free(e);
            e = next;
        }
        static_inst_table[i] = NULL;
    }
}

static unsigned int static_inst_hash(md_addr_t addr) {
    return ((addr >> 2) ^ (addr >> 4)) % STATIC_INST_HASH_SIZE;
}
//ADDED MAT FOR DYNAMIC INSTRUCTION COUNTING
...
fetch(int* current_fetch_context_ptr)
{
  ...
  for (i=0;
    ...
    //ADDED MAT FOR DYNAMIC INSTRUCTION COUNTING
      md_addr_t static_addr = contexts[current_fetch_context].fetch_regs_PC;
      unsigned int h = static_inst_hash(static_addr);
      StaticInstEntry* e = static_inst_table[h];
      enum md_opcode op_fetch;
      MD_SET_OPCODE(op_fetch, inst);
      while (e) {
          if (e->addr == static_addr) {
              e->count++;
              break;
          }
          e = e->next;
      ...
      if (!e) {
          e = (StaticInstEntry*)malloc(sizeof(StaticInstEntry));
          e->addr = static_addr;
          e->count = 1;
          strncpy(e->mnemonic, MD_OP_NAME(op_fetch), MAX_MNEMONIC_LEN-1);
          e->mnemonic[MAX_MNEMONIC_LEN-1] = '\0';
          e->next = static_inst_table[h];
          static_inst_table[h] = e;
      }
      total_dynamic_fetches++;
//ADDED MAT FOR DYNAMIC INSTRUCTION COUNTING
...
#include "machine.def"
  ...
  for (;;)
	...
	for(i=0;i<num_contexts;i++){
      ...
      //ADDED MAT FOR DYNAMIC INSTRUCTION COUNTING
    FILE* f = fopen("static_inst_profile.json", "w");
    if (f) {
        fprintf(f, "{\n");
        int first = 1;
        for (int i = 0; i < STATIC_INST_HASH_SIZE; i++) {
            for (StaticInstEntry* e = static_inst_table[i]; e; e = e->next) {
                double percent = (total_dynamic_fetches > 0)
                    ? (100.0 * (double)e->count / (double)total_dynamic_fetches)
                    : 0.0;
                if (!first) fprintf(f, ",\n");
                fprintf(f, "  \"0x%llx\": { \"mnemonic\": \"%s\", \"percent\": %.6f }",
                        (unsigned long long)e->addr, e->mnemonic, percent);
                first = 0;
            }
        }
        fprintf(f, "\n}\n");
        fclose(f);
        free_static_inst_table();
    } else {
        fprintf(stderr, "Could not open static_inst_profile.json for writing\n");
    }
    //ADDED MAT FOR DYNAMIC INSTRUCTION COUNTING
	    return;}
	}
